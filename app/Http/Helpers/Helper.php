...
function ads($alias)
{
    $ad = Advertisement::where([['alias', $alias], ['status', 1]])->first();
...
function dateFormat($date)
{
    ...
    $format = Date::parse($date)->format(dateFormatsArray()[settings('general')->date_format]);
    return $format;
}
...
{
    return asset(currentLanguage()->flag);
}
...
{
    $mailTemplate = MailTemplate::where([['lang', getLang()], ['alias', $alias]])->first();
    return $mailTemplate;
}
...
{
    $paymentGateway = PaymentGateway::where('alias', $alias)->first();
...
{
    $button = "";
    $buttonText = lang('Subscribe', 'plans');
    $buttonClass = $plan->isFeatured() ? ($plan->isFree() ? 'outline-light' : 'light') : ($plan->isFree() ? 'outline-primary' : 'primary');
    if (!$plan->isForGuests()) {
        $button = '<a href="' . route('login') . '"
        class="btn btn-' . $buttonClass . ' btn-lg w-100">' . $buttonText . '</a>';
    }
    if (auth()->user()) {
        $user = userAuthInfo();
        $subscription = $user->subscription;
        if ($user->isSubscribed() && $plan->id == $subscription->plan->id && $subscription->isFree()) {
            $button = '<button class="btn btn-' . $buttonClass . ' btn-lg w-100 disabled">' . lang('Renew', 'plans') . '</button>';
        } elseif ($user->isSubscribed() && $plan->id == $subscription->plan->id && !$subscription->isAboutToExpire()) {
            $button = '<button class="btn btn-' . $buttonClass . ' btn-lg w-100 disabled">' . lang('Renew', 'plans') . '</button>';
        } else {
            $type = "subscribe";
            ...
            <button class="btn btn-' . $buttonClass . ' btn-lg w-100 action-confirm">' . $buttonText . '</button>
            </form>';
        }
    }
    return $button;
