                ...
                <div class="card-v p-0">
                    <div class="settings-box">
                        <div class="settings-box-header border-bottom px-4 py-3">
                            <h5 class="mb-0">{{ lang('Payment History', 'account') }}</h5>
                        </div>
                        <div class="dash-table">
                                        ...
                                        @foreach ($transactions as $transaction)
                                            <tr>
                                                ...
                                                <td class="text-center">
                                                    {{ priceSymbol($transaction->total) }}
                                                </td>
                                                <td class="text-center"><span
                                                        class="d-none d-lg-inline">{{ $transaction->gateway->name ?? '--' }}</span>
                                                </td>
                                                <td class="text-center">{{ dateFormat($transaction->created_at) }}</td>
                                                <td class="text-center">
                                                    @if ($transaction->isPaid())
                                                        @if ($transaction->total > 0)
                                                            <span
                                                                class="badge bg-success">{{ lang('Paid', 'account') }}</span>
                                                        @else
                                                            <span
                                                                class="badge bg-primary">{{ lang('Done', 'account') }}</span>
                                                        @endif
                                                    @else
                                                        <span
                            ...
                            @else
                                <div class="py-5 text-center">
                                    ...
                                    <h4>{{ lang('Empty Section', 'account') }}</h4>
                                    <p class="mb-3 text-muted">
                                        {{ lang('This section is currently empty and has no content, please check it later.', 'account') }}
                                    </p>
                                </div>
                            @endif
                        </div>
